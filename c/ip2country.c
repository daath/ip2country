...
int EXPORT ip2c_init( const char* filename )
{
    FILE* fp;
    fileHeader *fhead;
...
short EXPORT ip2c_lookup( const char* ipnumber )
{
    unsigned long ip = 0;
...
int EXPORT ip2c_idx2country( const int idx, countryCode* theCode )
{
    theCode->code[ 1 ] = theCode->code[ 0 ] = '-';
    theCode->code[ 2 ] = '\0';
    if ( has_init == 1 && theIndex->countries >= idx && idx >= 0)
    {
        theCode->code[ 0 ] = countryName[ idx * 2 ];
        theCode->code[ 1 ] = countryName[ ( idx * 2 ) + 1 ];
        return 1;
    }
    else
    {
